...
#warning ("Don't forget to implement here as callbacks and not this crap")
  ...
  for (dr1bit = 0; dr1bit < 8; dr1bit++) {
    ...
    if (!(mask & ddr1) && !(dr2 & 1)) {
      ...
      for (column = 0; column < 7 && !found; column++) {
        ...
        if (st_keydown(scancode) && (dr3 & mask2)  // must be set (diode on)
            && (ddr3 & mask2))
          found++;
      }
      // test DR4
      for (column = 7; column < 15 && !found; column++) {
        mask2 = 1 << (column - 7);
        scancode = get_scancode(dr1bit, column);
        if (st_keydown(scancode) && (dr4 & mask2)  // must be set (diode on)
            && (ddr4 & mask2))
          found++;
      }
      if (found) {
        value &= ~mask;  // clear bit
        // DPRINTF("Read DR1 %X scancode %x\n",value,scancode);
      }
    }
  }
  //  ASSERT(value);
...
u_int offs;
{
  ...
  */
  if (!ddr4 && (ddr2 & 1) && (dr2 & 1)) {
    if (st_mouse_enabled()) {
      value = (value & (~0xF)) | (mouse_x_counter & 3) |
              ((mouse_y_counter & 3) << 2);
      // Add joystick 1
...
static port_putb(offs, value) u_int offs;
u_char value;
{
...
u_char(*ireg_getb_func[NIREGS]) P_((u_int offs)) = {
    /* 0x00 */
    0, 0, dr1_getb, dr2_getb, 0, 0, 0, dr4_getb, tcsr_getb, 0, 0, 0, 0, 0, 0, 0,
    /* 0x10 */
    0, trcsr_getb, rdr_getb, 0, 0};
...
int(*ireg_putb_func[NIREGS]) P_((u_int offs, u_char val)) = {
    /* 0x00 */
    0, 0, port_putb, port_putb, 0, 0, port_putb, port_putb, tcsr_putb, 0, 0,
    ocr_putb, ocr_putb, 0, 0, 0,
    /* 0x10 */
    0, trcsr_putb, 0, tdr_putb, 0};
