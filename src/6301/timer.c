...
u_int offs;
{
  ...
  if ((tcsr = ireg_getb(TCSR)) & OCF) tcsr_is_read = 1;
...
tcsr_putb(offs, value) u_int offs;
u_char value;
{
 ...
 */
ocr_putb(offs, value) u_int offs;
u_char value;
{
 ...
 */
timer_inc(ncycles) u_int ncycles;
{
  ...
  if (frc_old > frc_new) ireg_putb(TCSR, ireg_getb(TCSR) | TOF);
